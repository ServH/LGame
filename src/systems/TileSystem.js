...
export default class TileSystem {
          ...
          effects: {
            onTick: (entity) => {
              if (entity.type === 'player' && entity.isActive()) {
                entity.heal(0.01); // Pequeña curación pasiva
              }
            }
          },
    ...
    isAdjacentToPath(gridX, gridY) {
      // Necesitamos acceso al sistema de camino
      // Este es un método placeholder que se implementará cuando integremos sistemas
      if (!this.scene.pathSystem || !this.scene.pathSystem.path) {
        return false;
      }
      ...
      const path = this.scene.pathSystem.path;
        ...
        for (let dx = -1; dx <= 1; dx++) {
          for (let dy = -1; dy <= 1; dy++) {
            ...
            if (tile && tile.effects.onTick) {
              tile.effects.onTick(player, delta);
            }
          }
        }
      }
          ...
          for (let dx = -1; dx <= 1; dx++) {
            for (let dy = -1; dy <= 1; dy++) {
              ...
              if (tile && tile.effects.onTickEnemy) {
                tile.effects.onTickEnemy(enemy, delta);
              }
            }
          }
        }
